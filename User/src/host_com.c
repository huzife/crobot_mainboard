...
static EventGroupHandle_t host_com_event = NULL;
...
static void host_com_process() {
    ...
    EventBits_t bits = xEventGroupWaitBits(host_com_event, HOST_COM_TX_DONE, pdTRUE, pdTRUE, 500);
    if ((bits & HOST_COM_TX_DONE) != HOST_COM_TX_DONE) {
...
bool host_com_init(uint16_t buf_len) {
    ...
    host_com_cb.buf = pvPortMalloc(buf_len);
    host_com_cb.buf_len = buf_len;
    host_com_cb.data_len = 0;
    host_com_cb.buf[0] = 0xFE;
    host_com_cb.buf[1] = 0xEF;
    swsr_queue_init(&host_com_cb.rx_queue, 2 * buf_len);

    MX_USB_DEVICE_Init();

    host_com_event = xEventGroupCreate();
    if (!host_com_event) {
        PRINT_ERR("Failed to init host_com_event\n");
        return false;
    }

    xEventGroupSetBits(host_com_event, HOST_COM_TX_DONE);
...
void host_tx_callback() {
    ...
    xEventGroupSetBitsFromISR(host_com_event, HOST_COM_TX_DONE, &xHigherPriorityTaskWoken);
